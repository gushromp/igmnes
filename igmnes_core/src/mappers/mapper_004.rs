...
impl Mmc3 {
    pub fn new(rom: &Rom) -> Mmc3 {
        // MMC3 typically uses 8KB PRG RAM if any is present, though some carts
        // might have 0 or larger. We use the size from the header, defaulting to 8KB if 0.
        ...
        Mmc3 {
            vram: Ram::default(),
            prg_rom_bytes: rom.prg_rom_bytes.clone(),
    ...
    fn get_prg_rom_index(&self, index: u16) -> usize {
        let addr = (index - 0x8000) as usize;
        let prg_banks = self.prg_rom_bytes.len() / PRG_BANK_SIZE;

        let bank_index: usize = match addr / PRG_BANK_SIZE {
            0 => {
                // $8000-$9FFF
                if self.prg_bank_mode == 0 {
                    (self.r[6] as usize) % prg_banks
                } else {
                    prg_banks - 2 // P_L-1
                }
            }
            1 => (self.r[7] as usize) % prg_banks, // $A000-$BFFF
            2 => {
                // $C000-$DFFF
                if self.prg_bank_mode == 0 {
                    prg_banks - 2 // P_L-1
            ...
            3 => prg_banks - 1, // $E000-$FFFF (P_L)
            _ => unreachable!(),
        };

        let bank_offset = bank_index * PRG_BANK_SIZE;
        let addr_offset = addr & 0x1FFF; // Offset within the 8KB block

        (bank_offset + addr_offset) % self.prg_rom_bytes.len()
    }
